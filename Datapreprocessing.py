...
##            RatingPredict.rating_predict(numberOfUser, numberOfItem, mtx_np, mtx_ds, neighbor_user, k_user, ar, neighbor_item, k_item)
##          predict_user_and_item(numberOfUser, numberOfItem, mtx_np, mtx_ds, neighbor_user, k_user, ar, neighbor_item, k_item, att_weights_user, att_correct_user, input_n, hidden_n)


def predict_user_and_item(numberOfUser, numberOfItem, mtx_np,neighbor_user, k_user,
    ...
    for id in range(1, numberOfUser):
        ...
        for neighbor_id in neighbor_user[id]:
            # for i in range(len(mtx_np[neighbor_id])):
                # if(mtx_np[neighbor_id][i]==0 ):
                #     for node_neighbor_id in neighbor_user[neighbor_id]:
                #         if(mtx_np[node_neighbor_id][i] != 0):
                #             changemtx_np[neighbor_id][i] = mtx_np[node_neighbor_id][i]
                #             break
