...
class broadNet(object):
	...
	def weight_decode(self,Y_onehot):
		Y = []
		for i in range(Y_onehot.shape[0]):
			lis = np.ravel(Y_onehot[i,:]).tolist()
			weight = 0
			...
			for j in range(len(lis)):
				weight += (j+1)*(lis[j]-min(lis))/(max(lis)-min(lis))/total
			# print(((weight)))
			# print(round(weight))
			Y.append(round(weight)-1)
		return np.array(Y)
	...
	def weightPredict(self, testdata):
		#print(self.W.shape)
		testdata = self.normalscaler.transform(testdata)
		test_inputdata = self.transform(testdata)
		return self.weight_decode(test_inputdata.dot(self.W))
